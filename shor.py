...
def getAngles(a, N):
    ...
    for i in range(0, N):
        ...
        angles[N - i - 1] *= np.pi
    # Convert angles to standard Python floats
    return [float(angle) for angle in angles]

"""Creation of a doubly controlled phase gate"""
def ccphase(circuit, angle, ctl1, ctl2, tgt):
    circuit.cu1(float(angle / 2), ctl1, tgt)
    circuit.cx(ctl2, ctl1)
    circuit.cu1(float(-angle / 2), ctl1, tgt)
    circuit.cx(ctl2, ctl1)
    circuit.cu1(float(angle / 2), ctl2, tgt)
...
def cphiADD(circuit, q, ctl, a, n, inv):
    ...
    for i in range(0, n):
        if inv == 0:
            circuit.cu1(float(angle[i]), ctl, q[i])
        else:
            circuit.cu1(float(-angle[i]), ctl, q[i])
...
def ccphiADD(circuit, q, ctl1, ctl2, a, n, inv):
    ...
    for i in range(0, n):
        if inv == 0:
            ccphase(circuit, float(angle[i]), ctl1, ctl2, q[i])
        else:
            ccphase(circuit, float(-angle[i]), ctl1, ctl2, q[i])
